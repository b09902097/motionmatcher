...
class TextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        # reference_latents = latents.clone()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                def denoise_step(latents, scale):
                    loras = extract_lora_child_module(self.unet, target_replace_module=["Transformer2DModel", "TransformerTemporalModel"])
                    for lora_i in loras:
                        lora_i.scale = scale
                ...
                latents = denoise_step(latents, 1.0 if t >= min_timestep else 0.)
    ...
    @classmethod
    def step(self, noise_pred, timesteps, latents, scheduler, eta=0.0, generator=None):
        ...
        for idx in range(bsz):
            prev_latents.append(scheduler.step(noise_pred[idx:idx+1], timesteps[idx], latents[idx:idx+1], **extra_step_kwargs).prev_sample)
        prev_latents = torch.cat(prev_latents)
