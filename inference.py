...
from train import export_to_video, handle_memory_attention, load_primary_models, unet_and_text_g_c, freeze_models
...
def initialize_pipeline(
    ...
    handle_memory_attention(xformers, sdp, unet, None)
    ...
    lora_manager = LoraHandler(
        ...
        unet_replace_modules=["Transformer2DModel", "TransformerTemporalModel"],
        text_encoder_replace_modules=None,
        lora_bias=None
    )

    unet_lora_params, unet_negation = lora_manager.add_lora_to_model(
        True, unet, lora_manager.unet_replace_modules, 0, lora_path, r=lora_rank, scale=lora_scale)
...
def inference(
    ...
    with torch.autocast(device, dtype=torch.half):
        ...
        inverse = inverse_video(pipe, cached_latents['latents'].unsqueeze(0), num_steps).squeeze(0)
        ...
        for i in range(repeat_num):
            ...
            print(f"{out_name}_{random_seed}.mp4")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", type=str, default="models/zeroscope_v2_576w/",
    ...
    parser.add_argument("-s", "--num-steps", type=int, default=75, help="Number of diffusion steps to run per frame.")
    ...
    parser.add_argument("-np", "--noise_prior", type=float, default=0.3, help="Scale of the influence of inversion noise.")
    parser.add_argument("-ci", "--checkpoint_index", type=int, required=True,
                        help="The index of checkpoint, such as 300.")
    parser.add_argument("-rn", "--repeat_num", type=int, default=10,
                        help="How many results to generate with the same prompt.")
    parser.add_argument("-mn", "--min_timestep", type=int, default=500)
    ...
    #####
    # prompt = prompt + "8k, high detailed, best quality, film grain, Fujifilm XT3"
    ...
    lora_path = f"{args.checkpoint_folder}/checkpoint-{args.checkpoint_index}/lora"
